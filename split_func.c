...
char *swap_char(char *input, int bool)
{
	...
	else
	{
		for (id = 0; input[id]; id++)
		{
			input[id] = (input[id] == 16 ? '|' : input[id]);
			input[id] = (input[id] == 12 ? '&' : input[id]);
		}
	}
	return (input);
}
...
void add_nodes(seplist **head_s, linelist **head_l, char *input)
{
	...
	{
		if (input[id] == ';')
			add_sep_node_end(head_s, input[id]);
...
int split_commands(data_shell *datash, char *input)
{
	...
	seplist *head_s, *list_s;
	linelist *head_l, *list_l;
	int loop;
